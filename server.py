...
# Admin credentials are deliberately NOT written in this file. This
# file gets shared around for testing, and anything hardcoded here is
# readable by everyone holding a copy. They come from the environment,
# or from an admin_config.json living in the same per-user data
# directory as accounts.db - outside this folder, so it never travels
# with the code.
#
#   Environment (preferred - nothing on disk at all):
#       Linux/macOS:
#           ARENA_ADMIN_USERNAME=yourname ARENA_ADMIN_PASSWORD=yourpw python server.py
#       Windows (PowerShell):
#           $env:ARENA_ADMIN_USERNAME="yourname"
#           $env:ARENA_ADMIN_PASSWORD="yourpw"
#           python server.py
#
#   Or create the JSON file, whose exact path is printed on startup:
#       {"username": "yourname", "password": "yourpw"}
#
# With neither set, no admin account is created or refreshed and the
# server runs normally otherwise. If an admin account already exists in
# the database from an older version of this file, change its password
# by setting the credentials above to the same username and a new
# password, then restarting - the account is re-hashed on startup.
ADMIN_CONFIG_PATH = os.path.join(_default_data_dir(), "admin_config.json")


def _load_admin_credentials():
    """(username, password), or (None, None) if no admin is configured."""
    username = (os.environ.get("ARENA_ADMIN_USERNAME") or "").strip()
    password = os.environ.get("ARENA_ADMIN_PASSWORD") or ""

    if username and password:
        return username, password

    try:
        with open(ADMIN_CONFIG_PATH, "r") as config_file:
...
def init_db():
    ...
    if not ADMIN_USERNAME or not ADMIN_PASSWORD:
        # No admin configured - that's a supported setup, not an error.
        # Everything except the admin screens works exactly as normal.
        connection.close()
        print(
            "No admin credentials configured - skipping admin account.\n"
            "Set ARENA_ADMIN_USERNAME / ARENA_ADMIN_PASSWORD, or create "
            f"{ADMIN_CONFIG_PATH}"
        )
        return

    # Seed (or refresh) the configured admin account every startup, so
    # its credentials/privileges/stats are always right even if the DB
