...
bear_ride = None
monster = None
bear_traps = []      # Arachnid Ascendant's armed bear traps
...
# --- Arachnid Ascendant (Animalia variant) ---------------------------
# Twin-fang stabs that build into a frenzy, bear traps instead of
# arrows, and a giant spider in place of the T-Rex ultimate.
ARACHNID_VARIANT_ID = "animalia_arachnid"

STAB_RANGE = 96
STAB_DAMAGE = 9
STAB_COMBO_WINDOW = 0.9        # max gap between stabs before the combo drops
STAB_COMBO_TO_FRENZY = 3       # two stabs, and the third opens the frenzy
STAB_FRENZY_DURATION = 1.15
STAB_FRENZY_TICK = 0.085
STAB_FRENZY_DAMAGE = 4
STAB_FRENZY_AIM_DOT = 0.35     # how wide the flurry's cone is, as a dot()
STAB_FRENZY_HOLD_DRAIN = 16    # stamina/sec while the flurry is held open

BEAR_TRAP_THROW_DISTANCE = 150
BEAR_TRAP_ARM_TIME = 0.3
BEAR_TRAP_LIFE = 14
BEAR_TRAP_RADIUS = 30
BEAR_TRAP_DAMAGE = 26
BEAR_TRAP_ROOT_TIME = 2.0      # seconds locked in place
BEAR_TRAP_MAX_PER_OWNER = 3
BEAR_TRAP_SNAP_TIME = 0.4      # how long the shut jaws linger before clearing

SPIDER_LUNGE_TIME = 0.32
SPIDER_LUNGE_DISTANCE = 22
SPIDER_WALK_SPEED = 9          # leg cycle rate while the spider is moving
...
def active_variant_id():
    """The variant id actually in play for the currently selected
    element, or None. Guards against an id left over from a *different*
    element - selecting a character through story/tutorial entry points
    changes selected_element without going through the skin picker, and
    a stale id there used to hand the fighter a variant that isn't
    theirs, which silently fell back to the default kit."""
    if selected_variant_id is None:
        return None

    if not variants.variant_exists(selected_element, selected_variant_id):
        return None

    return selected_variant_id


def move_names_for(element, variant_id=None):
    """(short_name, long_name) for an element, with the equipped
    variant's moves swapped in where it has its own. Every screen that
    labels a character's moves goes through here, so a variant that
    replaces its short/long attacks is named correctly instead of still
    advertising the base kit."""
    data = ELEMENTS[element]
    variant = variants.get_variant(element, variant_id) if variant_id else None

    if variant is None:
        return data["short_name"], data["long_name"]

    return (
        variant.get("short_name", data["short_name"]),
        variant.get("long_name", data["long_name"]),
    )
...
def start_tutorial(element="fire"):
    global selected_element, selected_variant_id, tutorial_element, current_tutorial_steps
    ...
    if element != selected_element:
        # Don't carry a skin equipped on some other character into this
        # one's tutorial.
        selected_variant_id = None
...
def has_arachnid_kit(fighter):
    """True only for an Animalia fighter with Arachnid Ascendant equipped
    - the stab-frenzy / bear-trap / giant-spider kit."""
    return fighter.element == "animalia" and fighter.variant == ARACHNID_VARIANT_ID
...
def shoot_projectile(owner, direction, life=2.6):
    ...
    if has_arachnid_kit(owner):
        # Bear Trap: nothing travels - a trap is set on the ground ahead
        # and waits there instead. See update_bear_traps().
        place_bear_trap(owner, direction)
        return
...
def place_bear_trap(owner, direction):
    """Arachnid Ascendant's long range. Instead of firing an arrow, sets
    a bear trap on the ground a short way ahead. It arms after a beat,
    then snaps shut on the first opponent to walk into it."""
    trap_pos = Vector2(owner.pos) + direction.normalize() * BEAR_TRAP_THROW_DISTANCE
    trap_pos.x = max(ARENA.left + 20, min(ARENA.right - 20, trap_pos.x))
    trap_pos.y = max(ARENA.top + 20, min(ARENA.bottom - 20, trap_pos.y))

    # Keep the arena from filling up with traps - the oldest one this
    # fighter set is pulled up when they set one too many.
    owned = [trap for trap in bear_traps if trap["owner"] is owner]

    while len(owned) >= BEAR_TRAP_MAX_PER_OWNER:
        bear_traps.remove(owned.pop(0))

    bear_traps.append({
        "owner": owner,
        "team": owner.team,
        "pos": trap_pos,
        "arm_timer": BEAR_TRAP_ARM_TIME,
        "life": BEAR_TRAP_LIFE,
        "snap_timer": 0,
        "triggered": False,
    })

    add_effect(trap_pos, (200, 200, 205), 26, 0.25)


def update_bear_traps(dt):
    """Ages out traps, and snaps the armed ones shut on any opponent who
    steps inside. A sprung trap roots its victim for BEAR_TRAP_ROOT_TIME
    seconds and is then spent - one trap, one catch."""
    for trap in bear_traps[:]:
        if trap["triggered"]:
            # Already sprung - just holding the shut-jaws pose briefly.
            trap["snap_timer"] -= dt

            if trap["snap_timer"] <= 0:
                bear_traps.remove(trap)

            continue

        trap["life"] -= dt
        trap["arm_timer"] = max(0, trap["arm_timer"] - dt)

        if trap["life"] <= 0 or not trap["owner"].alive:
            bear_traps.remove(trap)
            continue
        ...
        for target in opponents_of(trap["owner"]):
            if in_shadow_pocket(target):
                continue

            if target.pos.distance_to(trap["pos"]) > BEAR_TRAP_RADIUS:
                continue

            hp_before = target.hp
            target.damage(BEAR_TRAP_DAMAGE * owner_damage_multiplier(trap["owner"]))
            target.root(BEAR_TRAP_ROOT_TIME)
            actual_dealt = max(0, hp_before - target.hp)

            if actual_dealt > 0:
                spawn_damage_number(target.pos, actual_dealt)
                trap["owner"].register_hit(actual_dealt)
                trap["owner"].ult = min(100, trap["owner"].ult + 12)
                play_sound(hit_sound)

                if not target.alive:
                    trap["owner"].register_kill()
                    spawn_kill_feed(f"{trap['owner'].name} eliminated {target.name}")

            add_effect(trap["pos"], (220, 220, 230), 44, 0.25)
            add_effect(target.pos, (215, 120, 235), 30, 0.2)

            trap["triggered"] = True
            trap["snap_timer"] = BEAR_TRAP_SNAP_TIME
            break
...
def apply_stab_hit(attacker, target, amount, ult_gain=7, sound=True):
    """One fang landing on one target - shared by the discrete stabs and
    by every tick of the frenzy. Returns the damage actually dealt."""
    hp_before = target.hp
    ...
    if actual_dealt > 0:
        ...
        attacker.ult = min(100, attacker.ult + ult_gain)
    ...
    add_effect(target.pos, (230, 160, 245), 30, 0.15)

    return actual_dealt


def perform_arachnid_stab(attacker, direction, targets):
    """Arachnid Ascendant's short range. Each press is a quick single-
    target stab; land a third one before the combo window lapses and it
    opens into a frenzy - and holding the attack button keeps that
    flurry running for as long as there's stamina for it (see
    update_stab_frenzies)."""
    now = pygame.time.get_ticks() / 1000
    last_stab = getattr(attacker, "stab_last_time", None)
    combo = getattr(attacker, "stab_combo", 0)

    if last_stab is None or now - last_stab > STAB_COMBO_WINDOW:
        combo = 0

    combo += 1
    attacker.stab_combo = combo
    attacker.stab_last_time = now

    if targets:
        target = min(targets, key=lambda fighter: fighter.pos.distance_to(attacker.pos))
        apply_stab_hit(attacker, target, STAB_DAMAGE * owner_damage_multiplier(attacker))

    add_effect(attacker.pos + direction.normalize() * 28, (215, 120, 235), 16, 0.12)

    if combo >= STAB_COMBO_TO_FRENZY:
        attacker.stab_combo = 0
        attacker.stab_frenzy_timer = STAB_FRENZY_DURATION
        attacker.stab_frenzy_tick = 0
        attacker.stab_frenzy_sound = 0
        add_effect(attacker.pos, (215, 120, 235), 55, 0.25)


def in_stab_frenzy(fighter):
    return getattr(fighter, "stab_frenzy_timer", 0) > 0


def update_stab_frenzies(dt):
    """Runs the flurry for every fighter in one - it's per-fighter state
    rather than a single global, so a bot fighting with the Arachnid kit
    and the player can both be mid-frenzy without stepping on each
    other. While the attack button is held (or a bot keeps its target in
    range) the flurry renews itself off stamina; let go, run dry, or get
    stunned and it winds down."""
    for fighter in [player] + bots:
        if not in_stab_frenzy(fighter):
            continue

        if not fighter.alive or fighter.stun_timer > 0 or in_shadow_pocket(fighter):
            fighter.stab_frenzy_timer = 0
            continue

        holding = getattr(fighter, "stab_frenzy_hold", False)

        if holding and fighter.has_stamina(STAB_FRENZY_HOLD_DRAIN * dt):
            # Held open: keep the flurry topped up and burn stamina for it.
            fighter.spend_stamina(STAB_FRENZY_HOLD_DRAIN * dt)
            fighter.stab_frenzy_timer = STAB_FRENZY_DURATION
        else:
            fighter.stab_frenzy_timer -= dt

        fighter.stab_frenzy_tick = getattr(fighter, "stab_frenzy_tick", 0) - dt
        fighter.stab_frenzy_sound = max(0, getattr(fighter, "stab_frenzy_sound", 0) - dt)

        direction = fighter.facing_direction

        if direction.length() == 0:
            direction = fighter.last_move_direction
        ...
        if fighter.stab_frenzy_tick <= 0:
            fighter.stab_frenzy_tick = STAB_FRENZY_TICK
            fighter.trigger_melee_swing(direction)

            dmg_mult = owner_damage_multiplier(fighter)
            hit_anyone = False
            ...
            for target in opponents_of(fighter):
                ...
                if to_target.length() == 0 or to_target.length() >= STAB_RANGE:
                    continue

                if direction.dot(to_target.normalize()) < STAB_FRENZY_AIM_DOT:
                    continue

                dealt = apply_stab_hit(
                    fighter, target, STAB_FRENZY_DAMAGE * dmg_mult, ult_gain=2, sound=False,
                )

                if dealt > 0:
                    hit_anyone = True

            # One hit sound per few ticks, or the flurry turns into noise.
            if hit_anyone and fighter.stab_frenzy_sound <= 0:
                play_sound(hit_sound)
                fighter.stab_frenzy_sound = 0.22

        if fighter.stab_frenzy_timer <= 0:
            fighter.stab_frenzy_timer = 0
...
def perform_short_attack(attacker, aim_direction):
    ...
    if in_stab_frenzy(attacker):
        # Already mid-flurry - the frenzy is driving the stabs itself.
        return
    ...
    if has_arachnid_kit(attacker):
        # Stab Frenzy: fast single-target jabs that snowball into a
        # flurry on the third one, rather than one heavy swing.
        perform_arachnid_stab(attacker, direction, targets)
        return
...
def trigger_ultimate(fighter):
    ...
    if fighter.element == "fire":
        ...
        spawn_facing = Vector2(spawn_offset)
        spawn_facing = spawn_facing.normalize() if spawn_facing.length() > 0 else Vector2(1, 0)
        ...
        monster = {
            ...
            "attack_cooldown": 0,
            # Arachnid Ascendant summons a giant spider instead - same
            # creature stats and AI, entirely different animal on screen.
            "kind": "spider" if has_arachnid_kit(fighter) else "trex",
            "facing": spawn_facing,
            "walk_phase": 0,
            "lunge": 0,
            "moving": False,
...
# --- Bot playstyles ---------------------------------------------------
# Each element fights differently rather than every bot running the same
# approach-and-shoot loop. "band" is the distance range a bot tries to
# hold: it closes when further away than band[1] and backs off when
# closer than band[0]. The flags after it are read in update_bot_ai().
BOT_PLAYSTYLES = {
    "fire": {"label": "Brawler", "band": (70, 150)},
    "metal": {"label": "Brawler", "band": (70, 150)},
    "average": {"label": "Brawler", "band": (70, 150)},
    "god": {"label": "Brawler", "band": (70, 150)},

    # Sniper: sits far back, shoots often, panics away from anything that
    # gets close instead of trading punches.
    "ice": {
        "label": "Sniper", "band": (300, 430), "melee_chance_mult": 0.15,
        "long_interval_mult": 0.7, "dash_style": "retreat", "dash_chance_mult": 1.8,
        "panic_range": 190,
    },

    # Regenerator: breaks off when hurt and parks on grass (where Natural
    # Respiration heals it fastest) until it's healthy again.
    "nature": {
        "label": "Regenerator", "band": (150, 260), "melee_chance_mult": 0.5,
        "dash_style": "retreat", "heal_below_hp": 0.6, "heal_until_hp": 0.9,
        "heal_zones": ("grass",),
    },

    # Bulwark: holds its ground, soaks hits, prefers standing on stone
    # (where its damage reduction kicks in) over chasing anyone.
    "earth": {
        "label": "Bulwark", "band": (60, 200), "melee_chance_mult": 1.35,
        "move_speed_mult": 0.6, "hold_ground": True, "dash_chance_mult": 0.35,
        "never_retreat": True, "anchor_zones": ("stone",),
    },

    # Ambusher: lurks in the map's shadow pockets (untouchable in there)
    # and only breaks cover once someone strays into pouncing range.
    "shadow": {
        "label": "Ambusher", "band": (60, 150), "melee_chance_mult": 1.3,
        "uses_pockets": True,
    },

    # Skirmisher: holds mid range where Lightning Bolt homes and chains,
    # and refuses to be dragged into melee.
    "lightning": {
        "label": "Skirmisher", "band": (210, 330), "melee_chance_mult": 0.3,
        "long_interval_mult": 0.7, "dash_style": "retreat", "chain_seeker": True,
    },

    # Berserker: dives in, and dives harder the more hurt it gets, since
    # An Eye For An Eye pays it for being at low health.
    "rage": {
        "label": "Berserker", "band": (0, 95), "melee_chance_mult": 1.6,
        "dash_style": "close", "dash_chance_mult": 2.2, "never_retreat": True,
        "rage_dive": True,
    },

    # Hunter: the default Animalia kit, leaning on Natural Taxi far more
    # than other bots dash, since the bear can attack on its own.
    "animalia": {
        "label": "Hunter", "band": (80, 170), "melee_chance_mult": 1.1,
        "dash_chance_mult": 3.0, "dash_style": "close",
    },

    # Conductor: cheap, always-available ultimate, so it pushes hard
    # whenever it has stock and backs off to rebuild when empty.
    "music": {"label": "Conductor", "band": (120, 220), "music_aggression": True},

    # Gale: never stands still. Constant strafing plus frequent dashes,
    # so it's a hard target rather than a stationary one.
    "wind": {
        "label": "Gale", "band": (140, 260), "strafe": 0.9, "never_still": True,
        "dash_chance_mult": 2.5, "melee_chance_mult": 0.6,
    },

    # Tactician: ordinary ranges, but picks its moments - presses when
    # the target is rooted/slowed/soaked, disengages when out of stamina
    # or badly hurt, and won't stand in lava to do it.
    "water": {"label": "Tactician", "band": (90, 200), "smart": True},
}

# Animalia's second playstyle. Bots that roll this one fight with the
# Arachnid Ascendant kit, so their long attack lays bear traps - they
# keep their distance and make the target walk into them.
ANIMALIA_TRAPPER_STYLE = {
    "label": "Trapper", "band": (200, 330), "melee_chance_mult": 0.4,
    "dash_style": "retreat", "dash_chance_mult": 1.2, "trapper": True,
    "smart": True,
}

ANIMALIA_TRAPPER_CHANCE = 0.5
SHADOW_AMBUSH_RANGE = 240
SHADOW_COVER_SEEK_RANGE = 620


def bot_profile(bot):
    """The playstyle for this bot, resolved once and cached on it.
    Animalia rolls between two: a dash-happy Hunter on the normal kit, or
    a Trapper carrying Arachnid Ascendant and its bear traps."""
    if getattr(bot, "playstyle_element", None) == bot.element:
        return bot.playstyle

    if bot.element == "animalia" and random.random() < ANIMALIA_TRAPPER_CHANCE:
        profile = ANIMALIA_TRAPPER_STYLE
        bot.variant = ARACHNID_VARIANT_ID
    else:
        profile = BOT_PLAYSTYLES.get(bot.element, BOT_PLAYSTYLES["fire"])

    bot.playstyle = profile
    bot.playstyle_element = bot.element
    return profile


def nearest_zone_of_type(position, zone_types):
    """Closest terrain zone of the given type(s), or None."""
    candidates = [zone for zone in TERRAIN_ZONES if zone["type"] in zone_types]

    if not candidates:
        return None

    return min(candidates, key=lambda zone: position.distance_to(zone["pos"]))


def update_shadow_ambush(bot, target, dt, multiplier):
    """Shadow's playstyle: treat the map's shadow pockets as cover. Slip
    into the nearest one (where nothing can touch it, and it can't
    attack either), wait there, and burst out the moment a target comes
    within pouncing range. Returns True when it has taken over movement
    for this frame - attacks still run normally afterwards."""
    if not SHADOW_POCKETS:
        return False

    pocket = min(SHADOW_POCKETS, key=lambda spot: bot.pos.distance_to(spot["pos"]))
    distance_to_target = bot.pos.distance_to(target.pos)
    hp_fraction = bot.hp / max(1, bot.max_hp)

    if in_shadow_pocket(bot):
        if distance_to_target < SHADOW_AMBUSH_RANGE:
            bot.ambushing = True

        if getattr(bot, "ambushing", False):
            # Break cover and jump them.
            bot.move(target.pos - bot.pos, dt, ARENA, multiplier * 1.15)
        else:
            # Settle into the middle of the pocket and wait.
            bot.move(pocket["pos"] - bot.pos, dt, ARENA, multiplier * 0.35)

        return True

    wants_cover = hp_fraction < 0.55 or distance_to_target > 340

    if wants_cover and bot.pos.distance_to(pocket["pos"]) < SHADOW_COVER_SEEK_RANGE:
        bot.ambushing = False
        bot.move(pocket["pos"] - bot.pos, dt, ARENA, multiplier)
        return True

    return False


def bot_avoid_hazard(bot, dt, multiplier):
    """Smart bots step out of terrain that's actively hurting them
    instead of standing in it while they fight."""
    zone = terrain_zone_at(bot.pos)

    if zone is None:
        return False

    hazardous = zone["type"] == "lava" and bot.element != "fire"

    if not hazardous:
        return False

    away = bot.pos - zone["pos"]

    if away.length() == 0:
        away = Vector2(1, 0)

    bot.move(away, dt, ARENA, multiplier * 1.2)
    return True


def update_bot_ai(bot, dt):
    profile = bot_profile(bot)
    target = nearest_target(bot)
    multiplier = terrain_speed_multiplier(bot) * profile.get("move_speed_mult", 1)
    ...
    if target is None:
        ...
        bot.move(bot.wander_direction, dt, ARENA, 0.5 * multiplier)
        return
    ...
    hp_fraction = bot.hp / max(1, bot.max_hp)
    band_min, band_max = profile["band"]
    approach_range = band_max - skill * 25
    retreat_range = 0 if profile.get("never_retreat") else max(0, band_min - skill * 10)
    melee_chance = (0.35 + skill * 0.5) * profile.get("melee_chance_mult", 1)
    attack_interval = (1.6 - skill * 1.05) * profile.get("long_interval_mult", 1)
    movement_handled = False

    # --- playstyle adjustments to the engagement band ------------------
    if profile.get("rage_dive"):
        # An Eye For An Eye pays out at low health, so the more hurt this
        # bot is, the more recklessly it closes.
        approach_range *= 0.55 + hp_fraction * 0.45
        melee_chance *= 1 + (1 - hp_fraction) * 0.6

    if profile.get("music_aggression"):
        # Symphony of Chaos is usable at any stock, so Music pushes while
        # it has some and retreats to rebuild once it's spent.
        if bot.ult > 0:
            approach_range = 110
            retreat_range = 0
            melee_chance *= 1.3
    ...
    if profile.get("smart"):
        impaired = (
            target.root_timer > 0 or target.stun_timer > 0
            or target.slow_timer > 0 or target.soak_timer > 0
        )

        if impaired:
            # They can't answer back properly - press the advantage.
            approach_range *= 0.6
            melee_chance *= 1.4
        elif bot.stamina < 25 or hp_fraction < 0.35:
            # Out of gas or badly hurt - buy time instead of trading.
            approach_range = max(approach_range, 300)
            retreat_range = max(retreat_range, 240)
            melee_chance *= 0.3

    if profile.get("heal_below_hp") is not None:
        # Nature: break off and sit on grass until healthy again.
        if hp_fraction < profile["heal_below_hp"]:
            bot.recovering = True
        elif hp_fraction > profile.get("heal_until_hp", 0.9):
            bot.recovering = False

        if getattr(bot, "recovering", False):
            melee_chance = 0
            grass = nearest_zone_of_type(bot.pos, profile.get("heal_zones", ("grass",)))
            zone_here = terrain_zone_at(bot.pos)
            on_heal_zone = zone_here is not None and zone_here["type"] in profile.get(
                "heal_zones", ("grass",)
            )

            if grass is not None and not on_heal_zone:
                bot.move(grass["pos"] - bot.pos, dt, ARENA, multiplier)
    ...
    if profile.get("uses_pockets") and not movement_handled:
        movement_handled = update_shadow_ambush(bot, target, dt, multiplier)

    if profile.get("smart") and not movement_handled:
        movement_handled = bot_avoid_hazard(bot, dt, multiplier)

    # --- movement -----------------------------------------------------
    if not movement_handled:
        panic_range = profile.get("panic_range")

        if panic_range is not None and distance < panic_range:
            # Sniper: anything this close is an emergency, not a target.
            bot.move(-direction, dt, ARENA, multiplier * 1.1)
        ...
        elif profile.get("hold_ground"):
            # Bulwark: in range and happy - plant, and only shuffle onto
            # nearby stone where its damage reduction applies.
            anchor = nearest_zone_of_type(bot.pos, profile.get("anchor_zones", ()))

            if anchor is not None and bot.pos.distance_to(anchor["pos"]) < 240:
                zone_here = terrain_zone_at(bot.pos)

                if zone_here is None or zone_here["type"] not in profile["anchor_zones"]:
                    bot.move(anchor["pos"] - bot.pos, dt, ARENA, multiplier * 0.7)

        if profile.get("strafe") or profile.get("never_still"):
            # Gale: keep sliding sideways so it's never a still target.
            bot.strafe_timer = getattr(bot, "strafe_timer", 0) - dt

            if bot.strafe_timer <= 0:
                bot.strafe_timer = random.uniform(0.5, 1.2)
                bot.strafe_sign = random.choice((-1, 1))

            if direction.length() > 0:
                sideways = Vector2(-direction.y, direction.x) * getattr(bot, "strafe_sign", 1)
                bot.move(sideways, dt, ARENA, multiplier * profile.get("strafe", 0.7))
    ...
    if (
        ...
        and random.random() < melee_chance
    ):
        ...
        bot.cooldowns["short"] = (0.5 - skill * 0.15) * (
            0.45 if has_arachnid_kit(bot) else 1
        )

    if has_arachnid_kit(bot):
        # Keep a bot's stab flurry alive while its target is still in
        # front of it, the same way holding the button does for a player.
        bot.stab_frenzy_hold = distance < STAB_RANGE

    if bot.cooldowns["long"] <= 0 and distance > 0:
        aim = direction

        if profile.get("trapper"):
            # Traps go where the target is heading, not where it is -
            # place_bear_trap() drops one a fixed way along this line.
            predicted = target.pos + target.last_move_direction * 70
            aim = predicted - bot.pos
    ...
    if (
        ...
        and random.random() < (0.015 + skill * 0.02) * profile.get("dash_chance_mult", 1)
    ):
        ...
        # pointed according to their playstyle.
        dash_style = profile.get("dash_style", "auto")

        if dash_style == "close":
            dash_direction = direction
        elif dash_style == "retreat":
            dash_direction = -direction
        else:
    ...
    if bot.element == "music":
        ...
        should_channel = bot.ult > 0 and distance < 340 and (
            already_channeling or random.random() < 0.04 + skill * 0.05
        )
        ...
        if random.random() < (0.5 + skill * 0.5) * profile.get("ult_chance_mult", 1):
...
def update_monster(dt):
    ...
    # Animation state - which way it's pointed, how far through its walk
    # cycle the legs are, and whether it's mid-lunge on a bite.
    monster["lunge"] = max(0, monster.get("lunge", 0) - dt)
...
def draw_encyclopedia():
    ...
    encyclopedia_variant = get_equipped_variant(encyclopedia_selected)
    encyclopedia_short, encyclopedia_long = move_names_for(
        encyclopedia_selected,
        encyclopedia_variant["id"] if encyclopedia_variant else None,
    )

    stat_rows = [
        ("Difficulty", data["difficulty"]),
        ("Short Range", encyclopedia_short),
        ("Long Range", encyclopedia_long),
        ("Ultimate", data["ultimate_name"]),
        ("Movement", data["movement_name"]),
        ("Passive", data["passive_name"]),
    ]

    if encyclopedia_variant is not None:
        stat_rows.insert(1, ("Skin", encyclopedia_variant["name"]))
...
def draw_character_select():
    ...
    for element, base_button in element_buttons.items():
        ...
        if unlocked:
            ...
            card_short, card_long = move_names_for(
                element, current_variant["id"] if current_variant else None,
            )

            if owned:
                skin_label = current_variant["name"] if current_variant else "Default"
                draw_text(card_short, button.x + 20, button.y + 96)
                ...
                draw_text(card_short, button.x + 20, button.y + 96)
                draw_text(card_long, button.x + 20, button.y + 118)
        else:
...
def draw_variant_picker():
    ...
    for variant_id, name, color, light_color, rect in options:
        ...
        # Spell out the moves each skin brings, so the difference
        # between kits is visible before committing to one.
        option_short, option_long = move_names_for(variant_picker_element, variant_id)

        for line_index, move_line in enumerate((option_short, option_long)):
            # Card is only 200px wide - clip rather than spill over it.
            while move_line and small_font.size(move_line)[0] > rect.width - 16:
                move_line = move_line[:-2] + "\u2026"

            move_label = small_font.render(move_line, True, (215, 215, 225))
            screen.blit(
                move_label,
                move_label.get_rect(center=(rect.centerx, rect.y + 156 + line_index * 20)),
            )

        if is_equipped:
            equipped_label = small_font.render("EQUIPPED", True, (140, 230, 150))
            screen.blit(equipped_label, equipped_label.get_rect(center=(rect.centerx, rect.y + 200)))
...
def draw_bear_trap(screen_pos, trap):
    """Jaws spread open while the trap is armed and waiting, slammed
    shut for a moment once something has walked into them."""
    if trap["triggered"]:
        openness = max(0, trap["snap_timer"] / BEAR_TRAP_SNAP_TIME) * 0.2
    elif trap["arm_timer"] > 0:
        openness = 1 - (trap["arm_timer"] / BEAR_TRAP_ARM_TIME)
    else:
        openness = 1

    radius = BEAR_TRAP_RADIUS
    plate_color = (72, 62, 58)
    metal_color = (185, 190, 200)
    center = Vector2(screen_pos)

    # Ground plate and chain stake.
    pygame.draw.circle(screen, plate_color, center, int(radius * 0.5))
    pygame.draw.circle(screen, (38, 34, 32), center, int(radius * 0.5), 2)
    pygame.draw.circle(screen, (110, 100, 95), center, int(radius * 0.18))

    for side in (-1, 1):
        jaw_center = center + Vector2(side * radius * 0.5 * openness, 0)

        pygame.draw.line(
            screen, metal_color,
            (jaw_center.x, jaw_center.y - radius * 0.62),
            (jaw_center.x, jaw_center.y + radius * 0.62),
            3,
        )

        for index in range(5):
            spread = (index / 4 - 0.5) * radius * 1.2
            base = jaw_center + Vector2(0, spread)
            tip = base - Vector2(side * (5 + 8 * openness), 0)
            pygame.draw.line(screen, metal_color, (base.x, base.y), (tip.x, tip.y), 2)

    if not trap["triggered"] and trap["arm_timer"] <= 0:
        # Faint armed ring, so a set trap is findable if you're looking.
        pygame.draw.circle(screen, (120, 80, 140), center, radius, 1)


def draw_giant_spider(screen_pos, monster_data):
    """Arachnid Ascendant's Ancient Monster: a massive spider with eight
    jointed legs that walk in an alternating gait, glowing red eyes, and
    a lunge forward-and-back on every bite."""
    pulse = (math.sin(pygame.time.get_ticks() / 1000 * 4) + 1) / 2

    facing = Vector2(monster_data.get("facing", Vector2(1, 0)))
    facing = facing.normalize() if facing.length() > 0 else Vector2(1, 0)
    perp = Vector2(-facing.y, facing.x)

    # Lunge: throws its weight forward on the bite, then rocks back.
    lunge_offset = 0

    if monster_data.get("lunge", 0) > 0:
        progress = 1 - (monster_data["lunge"] / SPIDER_LUNGE_TIME)
        lunge_offset = math.sin(progress * math.pi * 2) * SPIDER_LUNGE_DISTANCE

    center = Vector2(screen_pos) + facing * lunge_offset

    body_dark = (30, 16, 40)
    body_mid = (74, 36, 98)
    leg_color = (98, 52, 124)
    joint_color = (150, 90, 185)

    walk_phase = monster_data.get("walk_phase", 0)
    stepping = monster_data.get("moving", False)

    # --- eight legs, four a side, drawn behind the body ---------------
    for index in range(8):
        side = -1 if index < 4 else 1
        slot = index % 4
        out = facing.rotate((40 + slot * 42) * side)

        hip = center + out * 15
        span = 54 + slot * 5

        # Alternating tetrapod gait: neighbouring legs swing out of
        # phase, and opposite sides are offset again, so it never looks
        # like all eight are stepping together.
        phase = walk_phase + (slot % 2) * math.pi + (0 if side < 0 else math.pi / 2)
        swing = math.sin(phase) * (15 if stepping else 3)
        lift = abs(math.cos(phase)) * (10 if stepping else 2)

        foot = hip + out * span + facing * swing
        knee = hip + out * (span * 0.55) + facing * (swing * 0.4) - Vector2(0, 20 + lift)

        pygame.draw.lines(
            screen, leg_color, False,
            [(hip.x, hip.y), (knee.x, knee.y), (foot.x, foot.y)], 5,
        )
        pygame.draw.circle(screen, joint_color, (int(knee.x), int(knee.y)), 4)
        pygame.draw.circle(screen, body_dark, (int(foot.x), int(foot.y)), 4)

    # --- abdomen, thorax, head ----------------------------------------
    abdomen = center - facing * 32
    pygame.draw.circle(screen, body_mid, (int(abdomen.x), int(abdomen.y)), 30)
    pygame.draw.circle(screen, body_dark, (int(abdomen.x), int(abdomen.y)), 30, 3)

    marking_top = abdomen - facing * 14
    marking_bottom = abdomen + facing * 14
    pygame.draw.line(
        screen, (190, 60, 60),
        (marking_top.x, marking_top.y), (marking_bottom.x, marking_bottom.y), 4,
    )
    pygame.draw.line(
        screen, (190, 60, 60),
        (abdomen.x - perp.x * 11, abdomen.y - perp.y * 11),
        (abdomen.x + perp.x * 11, abdomen.y + perp.y * 11), 4,
    )

    thorax = center - facing * 4
    pygame.draw.circle(screen, body_dark, (int(thorax.x), int(thorax.y)), 22)
    pygame.draw.circle(screen, joint_color, (int(thorax.x), int(thorax.y)), 22, 2)

    head = center + facing * 18
    pygame.draw.circle(screen, body_dark, (int(head.x), int(head.y)), 14)

    # --- fangs, extended while lunging --------------------------------
    for side in (-1, 1):
        fang_base = head + perp * side * 6 + facing * 6
        fang_tip = fang_base + facing * (10 + max(0, lunge_offset) * 0.4) + perp * side * 3
        pygame.draw.line(
            screen, (225, 220, 235),
            (fang_base.x, fang_base.y), (fang_tip.x, fang_tip.y), 3,
        )

    # --- eight glowing red eyes ---------------------------------------
    glow_size = 40
    glow = pygame.Surface((glow_size, glow_size), pygame.SRCALPHA)
    pygame.draw.circle(
        glow, (255, 40, 40, int(60 + 60 * pulse)), (glow_size // 2, glow_size // 2), 16,
    )
    screen.blit(glow, (head.x - glow_size // 2, head.y - glow_size // 2))

    eye_color = (255, int(60 + 60 * pulse), int(60 + 40 * pulse))

    for row, (forward_offset, spacing, size) in enumerate(((9, 4, 3), (2, 7, 2))):
        for slot in range(4):
            offset = (slot - 1.5) * spacing
            eye_pos = head + facing * forward_offset + perp * offset
            pygame.draw.circle(screen, eye_color, (int(eye_pos.x), int(eye_pos.y)), size)


def draw_arachnid_stabbers(screen_pos, fighter):
    """Twin fang-daggers, thrust straight out on each stab and blurring
    into an unbroken flurry while Stab Frenzy is running."""
    frenzied = in_stab_frenzy(fighter)

    forward = (
        fighter.melee_swing_direction
        if fighter.melee_swing_timer > 0
        else fighter.facing_direction
    )
    ...
    if frenzied:
        thrust = abs(math.sin(pygame.time.get_ticks() / 1000 * 26)) * 18
    elif fighter.melee_swing_timer > 0:
        swing_t = 1 - (fighter.melee_swing_timer / fighter.melee_swing_duration)
        thrust = math.sin(swing_t * math.pi) * 16
    else:
        thrust = 0

    for side in (-1, 1):
        # The two fangs alternate, so the frenzy reads as a flurry of
        # stabs rather than both arms moving as one.
        if frenzied:
            blade_thrust = thrust if side > 0 else 18 - thrust
        else:
            blade_thrust = thrust if side > 0 else thrust * 0.55

        hand = screen_pos + perp * side * 7 + forward * 12
        tip = hand + forward * (14 + blade_thrust)
        back_a = hand + perp * side * 3
        back_b = hand - perp * side * 3

        pygame.draw.polygon(
            screen, (225, 220, 235),
            [(tip.x, tip.y), (back_a.x, back_a.y), (back_b.x, back_b.y)],
        )
        pygame.draw.circle(screen, (180, 90, 210), (int(tip.x), int(tip.y)), 2)

    if frenzied:
        for streak in range(3):
            streak_dir = forward.rotate(-30 + streak * 30)
            start = screen_pos + streak_dir * 22
            end = screen_pos + streak_dir * (30 + thrust)
            pygame.draw.line(screen, (215, 120, 235), (start.x, start.y), (end.x, end.y), 2)


def draw_arachnid_aura(screen_pos, variant_data):
    """Small spider legs creeping around the body - the skin treatment
    for Arachnid Ascendant, in place of Blue Inferno's flame licks."""
    t = pygame.time.get_ticks() / 1000

    for index in range(8):
        angle = (index / 8) * 360 + math.sin(t * 2 + index) * 6
        out = Vector2(1, 0).rotate(angle)
        hip = screen_pos + out * 15
        knee = hip + out * 7 - Vector2(0, 6 + math.sin(t * 6 + index) * 2)
        foot = knee + out * 7 + Vector2(0, 3)

        pygame.draw.lines(
            screen, variant_data["color"], False,
            [(hip.x, hip.y), (knee.x, knee.y), (foot.x, foot.y)], 2,
        )
...
def draw_fighter_weapon(screen_pos, fighter):
    ...
    elif element == "animalia" and fighter.variant == ARACHNID_VARIANT_ID:
        draw_arachnid_stabbers(screen_pos, fighter)
...
def draw_match():
    ...
    for trap in bear_traps:
        draw_bear_trap(world_to_screen(trap["pos"]), trap)
    ...
    for fighter in living_fighters():
        ...
        if eternal_night is not None and fighter is eternal_night["owner"]:
            ...
            if equipped_variant is not None:
                if has_arachnid_kit(fighter):
                    draw_arachnid_aura(screen_pos, equipped_variant)
                else:
    ...
    if monster is not None:
        ...
        if monster.get("kind") == "spider":
            draw_giant_spider(monster_screen_pos, monster)
            draw_text(
                "GIANT SPIDER",
                monster_screen_pos.x - 48,
                monster_screen_pos.y + 66,
                (230, 150, 250),
            )
            bar_width = 90
            bar_offset = 82
        ...
        bar_y = monster_screen_pos.y - bar_offset
    ...
    player_variant = variants.get_variant(player.element, player.variant) if player.variant else None

    if player_variant is not None:
        hud_short, hud_long = move_names_for(player.element, player.variant)
        # Sits under the stamina bar, clear of the minimap on the right.
        draw_text(
            f"KIT: {player_variant['name']} - {hud_short} / {hud_long}",
            480, 79, player_variant["light_color"], small_font,
        )
...
while running:
    ...
    if game_state == "game" and not game_over:
        ...
        if has_arachnid_kit(player):
            # Kirby-style: hold the attack button and the two opening
            # stabs roll straight into a flurry that keeps going for as
            # long as it's held and there's stamina for it. Set here,
            # outside the attack branches below, so it still updates on
            # frames that fall into a different branch entirely.
            player.stab_frenzy_hold = bool(mouse[0]) and not mouse[2]
        ...
        if combat_input_locked:
            ...
            # Stabs are quick and meant to be chained into the frenzy.
            player.cooldowns["short"] = 0.18 if has_arachnid_kit(player) else 0.38
            ...
            if player.element == "fire" and player.variant == "fire_blueflame":
                player.cooldowns["long"] = 1.4
            elif has_arachnid_kit(player):
                # Setting a trap is slower than loosing an arrow.
                player.cooldowns["long"] = 1.1
            else:
        ...
        update_specials(dt)
        update_monster(dt)
        update_bear_traps(dt)
        update_stab_frenzies(dt)
